...
output_dir 	= "conv_3l_108_N_summary_075_train_posibility_roc"
output_file = "conv_3l_108_fc_1024_N_summary_075_train_posibility_roc"

dataset_dir = "/home/dalinzhang/datasets/EEG_motor_imagery/2D_CNN_dataset/raw_data/"

with open(dataset_dir+"top108_shuffle_dataset_2D.pkl", "rb") as fp:
  	datasets = pickle.load(fp)
with open(dataset_dir+"top108_shuffle_labels.pkl", "rb") as fp:
...
def apply_conv2d(x, filter_height, filter_width, in_channels, out_channels, kernel_stride, phase_train):
	...
	conv_2d_bn = batch_norm(conv_2d, out_channels, phase_train)
...
def apply_fully_connect(x, x_size, fc_size, phase_train):
	...
	return tf.nn.elu(fc)
	# return max_out(tf.add(tf.matmul(x, fc_weight), fc_bias), fc_size)
...
# first CNN layer
conv_1 = apply_conv2d(X, kernel_height_1st, kernel_width_1st, input_channel_num, conv_channel_num, kernel_stride, phase_train)
# pool_1 = apply_max_pooling(conv_1, pooling_height, pooling_width, pooling_stride)
# print(pool_1.shape)

# second CNN layer
conv_2 = apply_conv2d(conv_1, kernel_height_2nd, kernel_width_2nd, conv_channel_num, conv_channel_num*2, kernel_stride, phase_train)
# pool_2 = apply_max_pooling(conv_2, pooling_height, pooling_width, pooling_stride)

# third CNN layer
conv_3 = apply_conv2d(conv_2, kernel_height_3rd, kernel_width_3rd, conv_channel_num*2, conv_channel_num*4, kernel_stride, phase_train)
# pool_3 = apply_max_pooling(conv_3, pooling_height, pooling_width, pooling_stride)

# fully connected layer
shape = conv_3.get_shape().as_list()

pool_2_flat = tf.reshape(conv_3, [-1, shape[1]*shape[2]*shape[3]])
fc = apply_fully_connect(pool_2_flat, shape[1]*shape[2]*shape[3], fc_size, phase_train)
...
with tf.Session(config=config) as session:
	...
	for epoch in range(training_epochs):
		...
		if(epoch%1 == 0):
			...
			if (np.mean(test_accuracy) > 0.884):
				break
	test_accuracy 	= np.zeros(shape=[0], dtype=float)
	test_loss 		= np.zeros(shape=[0], dtype=float)
