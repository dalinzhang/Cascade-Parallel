...
with open(dataset_dir+"top108_shuffle_dataset_1D.pkl", "rb") as fp:
  	datasets = pickle.load(fp)
with open(dataset_dir+"top108_shuffle_labels_1D.pkl", "rb") as fp:
...
def apply_conv1d(x, filter_width, in_channels, out_channels, kernel_stride, phase_train):
...
# first CNN layer
conv_1 = apply_conv1d(X, kernel_width_1st, input_channel_num, conv_channel_num, kernel_stride, phase_train)
# pool_1 = apply_max_pooling(conv_1, pooling_height, pooling_width, pooling_stride)
# print(pool_1.shape)

# second CNN layer
conv_2 = apply_conv1d(conv_1, kernel_width_2nd, conv_channel_num, conv_channel_num*2, kernel_stride, phase_train)
# pool_2 = apply_max_pooling(conv_2, pooling_height, pooling_width, pooling_stride)
# print(conv_2.shape)

# third CNN layer
conv_3 = apply_conv1d(conv_2, kernel_width_3rd, conv_channel_num*2, conv_channel_num*4, kernel_stride, phase_train)
...
fc = apply_fully_connect(pool_2_flat, shape[1]*shape[2], fc_size, phase_train)
...
with tf.Session(config=config) as session:
	...
	for epoch in range(training_epochs):
		...
		if(epoch%1 == 0):
			...
			if (np.mean(test_accuracy) > 0.862):
				break
	test_accuracy 	= np.zeros(shape=[0], dtype=float)
	test_loss 		= np.zeros(shape=[0], dtype=float)
